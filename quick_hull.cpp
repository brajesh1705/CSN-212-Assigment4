...
void Finalparhull(vector<point> &arr, vector<point> &result, point p1, point pn)
{
    ...
    {    
        ...
        Finalparhull(s11, result, p1, pmax); //recursively search in s11
        Finalparhull(s12, result, pmax, pn); //recursively search in s12
    }
}
int main(){
    int n;
    ...
    Finalparhull(P, upper, P[0], P[n-1]);   //upper hull, line direction p1-->pn
    Finalparhull(P, lower, P[n-1], P[0]);   //lower hull, line direction pn-->p1
